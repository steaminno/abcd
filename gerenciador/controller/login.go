...
func LoginPOST(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Println("result password ", result.Password)
	...
	if err == model.ErrNoResult {
		...
		sess.Save(r, w)
	} else if result.Password == password { //else if passhash.MatchString(result.Password, password) {
		if result.Status != "active" {
			...
			sess.Values["uname"] = result.Username
			sess.Save(r, w)
